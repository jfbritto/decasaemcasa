    ...
    <nav class="bg-white shadow-md border-b border-gray-200" x-data="{ open: false }">
            ...
            <div class="flex justify-between h-16">
                <div class="flex">
                    <div class="flex-shrink-0 flex items-center">
                        <a href="{{ auth()->check() && auth()->user()->isAdmin() ? route('admin.dashboard') : route('inscricao.create') }}" class="text-2xl font-bold text-indigo-600">
                            De Casa em Casa
                        </a>
                    </div>
                    <!-- Menu Desktop -->
                    <div class="hidden sm:ml-6 sm:flex sm:space-x-8">
                        @auth
                            @if(auth()->user()->isAdmin())
                                <a href="{{ route('admin.events.index') }}" class="{{ request()->routeIs('admin.events.*') ? 'border-indigo-500 text-gray-900' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700' }} inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium">
                                    Encontros
                                </a>
                ...
                <div class="hidden sm:flex sm:items-center">
                    ...
                    @else
                        {{-- <a href="{{ route('login') }}" class="text-gray-500 hover:text-gray-700 mr-4 text-sm">Admin</a> --}}
                    @endauth
                </div>
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="sm:hidden" style="display: none;">
            <div class="pt-2 pb-3 space-y-1 border-t border-gray-200">
                @auth
                    @if(auth()->user()->isAdmin())
                        <a href="{{ route('admin.events.index') }}" @click="open = false" class="{{ request()->routeIs('admin.events.*') ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'border-transparent text-gray-500 hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700' }} block pl-3 pr-4 py-2 border-l-4 text-base font-medium">
                            Encontros
                        </a>
            ...
            <div class="pt-4 pb-3 border-t border-gray-200">
                ...
                @else
                    {{-- <div class="space-y-1 px-2">
                        <a href="{{ route('login') }}" @click="open = false" class="block px-3 py-2 rounded-md text-base font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-50">
                            Admin
                        </a>
                    </div> --}}
                @endauth
